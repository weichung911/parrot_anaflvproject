...
def _main():
    ...
    while True:
        ...
        # lmList = detector.findPosition(imS, draw=False)
        ...
        blob = cv2.dnn.blobFromImage(imS, 1 / 255, (IMG_WIDTH, IMG_HEIGHT),
                                     [0, 0, 0], 1, crop=False)
        ...
        faces = post_process(imS, outs, CONF_THRESHOLD, NMS_THRESHOLD)
        ...
        for (i, (txt, val)) in enumerate(info):
            text = '{}: {}'.format(txt, val)
            cv2.putText(imS, text, (10, (i * 20) + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLOR_RED, 2)

        # Save the output video to file
        if args.image:
            cv2.imwrite(os.path.join(args.output_dir, output_file), imS.astype(np.uint8))
        else:
            video_writer.write(imS.astype(np.uint8))

        cv2.imshow(wind_name, imS)
